...
a related trading project called internal-research-service. It never trades and never touches a trading
...
- Collects macro and market news from RSS feeds (news feeds that update automatically) and
  scoped web searches.
- Compares that news, read-only, against how internal-research-service's paper-traded (simulated, no real
  money) strategies actually behaved, using a local copy of internal-research-service's `paper.db` database.
- Proposes candidate hypotheses (testable research ideas) for a human to review, at most once a
  week. A human then runs any idea worth pursuing through internal-research-service's own `/spec-gather`
  planning pipeline.
...
- It never invokes internal-research-service's evaluation pipeline.
...
- This repo's service account, `internal-monitor-service`, can read that snapshot because it belongs to the
  `paper-readers` group. It cannot reach `/home/internal-research-service` at all.
...
sudo scripts/deploy.sh              # deploy under the internal-monitor-service service account and enable the daily timer
sudo -u internal-monitor-service bash scripts/smoke_e2e.sh   # verify the live read path
```

## Observability

This follows section 18 of internal-infra's CONVENTIONS.md file (internal-infra is the shared
...
yet — that needs a change on the internal-infra side (adding this service to `config.alloy`'s
...
also live in internal-infra's `alert-rules.yml` and haven't been added yet.
