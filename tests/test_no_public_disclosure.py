...
2. The literal string ``desktop.example.internal`` — the real fleet's SSH alias for the deploy host. Docs and
   scripts must refer to "the deploy host" / an env-var-configured hostname instead.
3. Any of the real internal-research-service paper-trading strategy sleeve names. These are the operator's
...
_DESKTOP_AGENT_RE = re.compile(r"desktop.example.internal")

# The real live internal-research-service paper-trading strategy sleeve names (per the public-readiness audit).
...
def test_no_desktop_agent_alias_in_tracked_source():
    ...
    assert not violations, (
        "The real fleet's 'desktop.example.internal' SSH alias was found in tracked source — this must "
...
def test_no_real_strategy_sleeve_names_in_tracked_source():
    ...
    assert not violations, (
        "A real internal-research-service strategy sleeve name was found in tracked source — only generic "
