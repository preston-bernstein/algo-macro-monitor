...
def paper_db(tmp_path) -> str:
    ...
    conn.executemany(
        ...
        [
            ("2026-07-04", "cross_asset_trend", "SPY", 0.25, 100, 55000.0, 0.98),
            ("2026-07-04", "yield_curve_carry", "TLT", -0.10, 40, 4000.0, 0.95),
            ("2026-07-01", "cross_asset_trend", "SPY", 0.30, 120, 66000.0, 0.97),  # decoy date
        ],
    )
    conn.executemany(
    ...
    conn.executemany(
        ...
        [
            ("2026-07-04", "cross_asset_trend", 1, 1.2, 0.9, 12, 0.8, 0.1, 0.7, "ok"),
            ("2026-07-01", "cross_asset_trend", 0, 0.3, 0.1, 4, 0.2, 0.6, 0.4, "fail"),  # decoy
        ],
    )
    conn.commit()
    conn.close()
