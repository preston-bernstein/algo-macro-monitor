...
- Local clone of internal-monitor-service with venv activated: `python3 -m venv .venv && . .venv/bin/activate && pip install -e ".[dev]"`
...
**Test**: Run `ssh desktop.example.internal "sudo -u internal-monitor-service sqlite3 /home/internal-monitor-service/app/data/macro_monitor.db 'SELECT name, url_or_query, fetchable FROM sources;'"` and verify all `url_or_query` results start with `https://`. If any are `http://`, capture them in a note to Preston and do not proceed.
**Depends on**: Step 1
**Parallelizable**: Yes
