...
and already ships to one real consumer (fashion-monitor, via subprocess since that consumer is
TypeScript). algo-macro-monitor and feed-commons are both Python in the same dev environment, so
the hand-rolled path is duplicated maintenance with no cross-language justification. The person
who has this problem is whoever maintains algo-macro-monitor's collection path (Preston) and,
...
unattended (`systemd/macro-monitor-collect.timer`, `algo-macro` service user, desktop host) and
...
- `algo-macro-monitor`'s deterministic RSS collector (`fetch_and_log_rss`, `collect_rss`) — the
  direct caller being retrofitted.
- The `algo-macro` service user running `macro-monitor-collect.timer` unattended on the desktop
...
- Access to the production desktop deploy host's SQLite DB for the AC-12/AC-13/AC-15/AC-16
  ...
  `/home/algo-macro/app/data/macro_monitor.db` read-only via `sudo -u algo-macro sqlite3 ...`.
...
- Migrating fashion-monitor's subprocess-based feed-commons integration to anything — that
