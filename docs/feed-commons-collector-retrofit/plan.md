...
needed, unlike internal-monitor-app's TypeScript case). `poll()` already returns fully-normalized,
...
- **In-process integration, not a subprocess boundary.** Unlike internal-monitor-app's TypeScript case
...
catch-all internal-monitor-app's `bulletin-poll.ts` added for the identical reason.
...
- **Deploy-key access to feed-commons is unverified — converged finding from 3 independent
  reviewers.** Nothing in this plan verifies that the deploy-host service user (`internal-monitor-service`, on
  ...
  before merge/deploy**: confirm the CI environment and the `internal-monitor-service` service user on the
