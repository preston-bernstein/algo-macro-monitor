...
None. This implementation creates a new sibling repository (`internal-monitor-service`) from scratch, following the `internal-corpus-service` pattern. No existing work must be done first; however, the implementer must have:
- SSH key configured for `desktop.example.internal` access (per home lab setup in CLAUDE.md)
- Ability to run `sudo -n` on desktop.example.internal without password prompt
...
### Step 1: Create internal-monitor-service repo skeleton
...
**Test**: `cd ~/dev/internal-monitor-service && git status` shows clean repo; `grep algo.factory pyproject.toml` returns nothing (no algo_factory dependency).
**Depends on**: none
**Parallelizable**: No
...
**What**: Implement FR-05/FR-06 read-only correlation logic; open paper.db via SSH subprocess (`ssh desktop.example.internal sudo -n python3 -c "import sqlite3; ..."`); fetch targets/marks/gate_results rows for each observed_date (using correct column names: targets.date, marks.date, gate_results.run_date); store results as JSON in correlations table.
**Files**: src/macro_monitor/correlator.py
**Test**: Mock test: `python -c "from macro_monitor.correlator import build_query; q = build_query('targets', '2026-07-04'); assert 'date' in q"` and `q2 = build_query('gate_results', '2026-07-04'); assert 'run_date' in q2` (no actual SSH needed for unit test); integration test requires desktop.example.internal access and will be verified during deploy.
**Depends on**: Step 2, Step 4
**Parallelizable**: Yes
...
**What**: Create macro-monitor-collect.service (Type=oneshot, runs `~internal-monitor-service/bin/macro-monitor correlate --date <TODAY>` after RSS collection) and macro-monitor-collect.timer (daily, 06:30 UTC, Persistent=true, matches paper-track.timer pattern). Service assumes internal-monitor-service service user with home directory /home/internal-monitor-service/.
...
**What**: Create deployment script that: (a) creates internal-monitor-service nologin service user on desktop, (b) copies pyproject.toml and src/ to /home/internal-monitor-service/app/, (c) installs dependencies in service user's venv, (d) copies systemd files to /etc/systemd/system/, (e) enables and starts timer, (f) verifies timer runs without error on first run. Mirrors internal-corpus-service/scripts/deploy.sh pattern.
**Files**: scripts/deploy.sh
**Test**: `bash scripts/deploy.sh --dry-run` shows all copy/install commands without executing; `ssh desktop.example.internal "sudo /home/internal-monitor-service/app/bin/macro-monitor --version"` works (after non-dry-run deploy).
...
### Step 14: Update internal-research-service docs/DECISIONS.md integration point
...
**Files**: docs/DECISIONS.md (in internal-research-service repo — this worktree's own root, not a parent directory)
...
- **Steps 1-4 (skeleton, schema, test, CLI)**: Delete ~/dev/internal-monitor-service directory; revert any changes to internal-research-service docs/ if already made.
...
- **Steps 11-13 (systemd, deploy, slash-commands)**: Delete systemd/ directory and scripts/deploy.sh; delete .claude/commands/ files; on desktop, run `sudo systemctl disable macro-monitor-collect.timer && sudo rm -rf /etc/systemd/system/macro-monitor-* /home/internal-monitor-service/`.
- **Step 14 (DECISIONS.md)**: Revert the single entry from internal-research-service docs/DECISIONS.md.

If any step fails before Step 14, the internal-research-service repo is unmodified; rollback is isolated to the new internal-monitor-service repo (which can be deleted entirely). If Step 14 is committed before later steps fail, revert the DECISIONS.md commit separately.
