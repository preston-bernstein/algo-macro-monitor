...
- **[AUTH] No signing/authentication on the `ingest` CLI itself (any local process could inject
  ...
  single-host deployment model (the same host already trusts whoever can run code as the `internal-monitor-service`
...
- Sibling-repo-vs-subfolder justification, WAL writer-locking gap, over-schema'd
  ...
  `internal-corpus-service` precedent Preston has used consistently today, and a second data point genuinely
...
spec is ready for a `/new-story` implementation pass, either in internal-research-service's own campaign or (per
