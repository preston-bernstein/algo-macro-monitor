...
Build this as a new sibling repo, `internal-monitor-service` (confirmed unclaimed under `~/dev/`), following the
`internal-corpus-service` precedent exactly: separate git hygiene wall, separate service user, separate deploy, zero
...
```
 desktop.example.internal (systemd, service user internal-monitor-service, mirrors internal-research-service's paper-track pattern)
 ┌────────────────────────────────────────────────────────────────────────┐
 ...
 │   /home/internal-research-service/app/data/paper.db  (mode=ro, never writes)        │
 │        │  writes                                                       │
 │        v                                                                │
 ...
 └────────────────────────────────────────────────────────────────────────┘
        ...
        manually runs, inside internal-research-service:  /spec-gather <slug>
                          │
                          v
...
(`command="/home/internal-monitor-service/app/bin/ro_query.py"` in that identity's `authorized_keys`, so the key can
...
New SQLite database, `data/macro_monitor.db`, owned by `internal-monitor-service` (gitignored, WAL mode — same
pattern as `algo_factory`'s `research.db`/`paper.db`, but this repo's copy is explicitly **not** versioned,
unlike internal-research-service's deliberately-committed data files, because this is raw scraped content per FR-15).
...
- `macro-monitor correlate --date YYYY-MM-DD` — read-only `paper.db` query per FR-05/FR-06/FR-17/FR-18/
  FR-19. When invoked by the systemd-timer process (already running ON desktop.example.internal), this is a LOCAL
  `sudo -n <ro_query_script>` call — no SSH self-loopback. When invoked from the schedule-skill-hosted
  routines (running on the Mac), this is a genuine `ssh desktop.example.internal` hop to the same local call on the
...
- Report artifact: `reports/<slug>.md` — human-readable rendering of one `candidate_hypotheses` row,
  ...
  automatically (FR-10).
- Hand-off contract (manual, by design): Preston reads `reports/<slug>.md`, then in an internal-research-service Claude
  Code session runs `/spec-gather <slug>`. No code in `internal-monitor-service` names or invokes `/spec-gather`,
  `/spec-challenge`, the backtest runner, or any gate-evaluation function (FR-10/FR-11, acceptance criteria 6 & 10).

## Integration points

internal-research-service side (spec-only — no `src/` changes):
...
- Explicitly **not touched**: anything under `src/algo_factory/` — no new module, no edit, per FR-12/FR-13.
  The only internal-research-service artifact this tool reads is `data/paper.db` on the desktop, read-only, over SSH.

internal-monitor-service side (new sibling repo, proposed layout, paths relative to its own root):
- `README.md` — project overview and quickstart guide for contributors/deployers.
- `pyproject.toml` — own dependency set; **must not** list `internal-research-service`/`algo_factory` as a dependency —
...
- `config.example.yaml` / `config.yaml` (gitignored) — desktop.example.internal SSH target, cadence overrides, symbol
...
- `scripts/deploy.sh` — desktop deploy under a new dedicated nologin service user (e.g. `internal-monitor-service`),
  mirroring `internal-corpus-service/scripts/deploy.sh` and `internal-research-service`'s existing service-user convention;
...
- **`feedparser`** (new dep, `internal-monitor-service` only) for RSS/Atom parsing — handles the real-world feed
...
- **SSH subprocess to `desktop.example.internal`** for `paper.db` reads, via a DEDICATED scoped credential (FR-17)
...
- **`paper.db` access is a fragile external dependency.** Every correlation/review run depends on
  `desktop.example.internal` being reachable, the dedicated scoped credential (FR-17) continuing to work
  non-interactively, and the `internal-research-service` service user's ownership/permissions on `paper.db` staying
...
- **Symbol-universe drift.** The tracked-strategy instrument list is a manually-synced snapshot in
  `internal-monitor-service`'s own config, not a live read of `paper.db`'s `targets`/`marks` distinct symbols
  (deliberately, to avoid a live dependency edge into internal-research-service). If Preston adds a new sleeve/instrument
