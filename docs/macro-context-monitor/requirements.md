...
Algo-factory's paper-traded strategies (`cross_asset_trend`, `cs_momentum`, `equity_momentum_mtum`,
`yield_curve_carry`, and the other sleeves tracked in `paper.db`) run against a market context —
...
- **algo-factory `/spec-gather` → `/spec-challenge` → backtest → gate pipeline** — the sole
...
- **A future sibling repo (tentatively `algo-macro-monitor`)** — likely code host for the scraper/
...
read-only, via `sudo -n` + `python3 -c "import sqlite3; ..."` on `desktop-agent`, owned by service
user `algo-factory`) and attach: (a) all `targets` rows where `targets.date = observed_date`
...
repo hosts the code, matching the `algo-corpus` precedent (raw scanned content, OCR output, and
...
- Log storage growth is UNBOUNDED OVER TIME (append-only, per FR-14, with no retention/pruning
  ...
  `paper.db`'s `targets`/`marks` tables (cross_asset_trend, cs_momentum, cs_momentum_wide,
  cs_trend_cross_asset, equity_momentum_mtum, individual_momentum, industry_momentum,
  portfolio_ensemble, vrp_spy_timing, yield_curve_carry and their instruments), not an open-ended
...
- Read access to `paper.db` must use the existing pattern: `ssh desktop-agent`, `sudo -n`, and
  `python3 -c "import sqlite3; ..."` (no `sqlite3` CLI installed on the desktop box; the DB is not
  world-readable, owned by service user `algo-factory`).
...
- Working assumption (to be validated by `/spec-challenge`, not treated as final): code lives in a
  new sibling repo (tentatively `algo-macro-monitor`), following the `algo-corpus` precedent of a
  separately git-hygiened, independently-deployed concern; the spec itself lives in
  `docs/macro-context-monitor/` inside algo-factory for discoverability regardless of where the
  code ends up.
- Any cron/systemd-timer scheduling should mirror algo-factory's existing paper-tracking timer
