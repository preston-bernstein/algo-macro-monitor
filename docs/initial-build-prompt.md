You are an unattended Claude Code session running in `/home/preston/repos/algo-macro-monitor` on
...
2. `docs/DECISIONS.md` in THIS repo — it resolves two things that deviate from plan.md's text and
   ...
   - FR-17 (paper.db read access) is via a periodic snapshot at `/srv/paper-share/paper.db`
     (refreshed every 15 min, transaction-consistent, read-only for the `algo-macro` service
   ...
   - The live paper.db path (for your own understanding/testing only — you should NOT read it
     directly, only the snapshot) is `/home/algo-factory/app/data/paper.db`.
   - This repo's own service user is `algo-macro` (nologin, home `/home/algo-macro`).
...
PROJECT SETUP: mirror `algo-factory`'s conventions reasonably (`pyproject.toml`, `src/` layout,
...
(`git worktree add /tmp/wt-<slug> -b work-<slug>`) exactly like algo-factory's continuous-campaign
...
GIT / PR DISCIPLINE (same non-negotiables as algo-factory):
...
- Never touch `/home/algo-factory/` or its live `paper.db` — read-only via the `/srv/paper-share`
  snapshot only, and even that only through the `algo-macro` group-read grant already provisioned.
...
pass — the exact convention algo-factory's own DECISIONS.md follows. Be honest: a partial pass with
