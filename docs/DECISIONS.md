...
A fleet-wide `home-infra` audit (`CONVENTIONS.md` §18) flagged this repo as zero-coverage
...
**Deliberately not done in this pass:** Loki log shipping (a `home-infra`-side Lane B change —
...
did-nothing gate against the new metrics) — both live in `home-infra`, not here. Nothing in this
...
- **`macro-monitor review` had never been invoked, automatically or manually, since deploy —
  ...
  research sweep in the sibling `algo-factory` repo asked whether this repo's hypothesis reports
...
- **Ran the first review by hand** (`macro-monitor review --since 2026-05-21`, run_id 1, as the
  `algo-macro` service user), reading the full backlog rather than the default 7-day window since
  ...
  coincidences with whatever crypto-momentum symbols (`cs_momentum_wide`) happened to be in
...
- **Deliberately not done in this pass:** wiring `/macro-monitor-review` onto an actual recurring
  ...
  live `/home/algo-macro/app/data/macro_monitor.db` or the `/srv/paper-share/paper.db` snapshot, so
  the review would have nothing to read. The right shape is a local systemd timer + `claude -p`
  invocation as the `algo-macro` service user, matching the pattern already proven in
  `algo-factory/scripts/desktop_campaign_cycle.sh` (preflight, structured logging, fail-closed exit
  ...
  scraped content. Now that the metric exists, the concrete next step is a `home-infra` alert on
...
- **`src/macro_monitor/log.py` and `src/macro_monitor/config.py` are now thin wrappers around the
  ...
  `algo-factory/pyproject.toml` already uses for `scraper-commons`). `fleet-logging` was built as
...
the same way `algo-macro`'s desktop deploy did before its deploy key was provisioned (see above).
Fixed the same way `fashion-monitor`/`resale-inventory` already solved this for `credential-crypto`:
