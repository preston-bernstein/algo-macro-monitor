#!/usr/bin/env bash
# Deploy Macro Context Monitor to the desktop under the algo-macro nologin service user.
#
# Mirrors algo-corpus/scripts/deploy.sh and algo-factory's service-user convention. Per
# docs/DECISIONS.md, the FR-17 paper.db read path is GROUP-READ (algo-macro ∈ paper-readers reads
...
# 2. FR-17 read-path check: algo-macro must be in paper-readers and able to read the snapshot.
